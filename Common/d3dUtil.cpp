...
Microsoft::WRL::ComPtr<ID3D12Resource> d3dUtil::CreateDefaultBuffer(
  ...
  auto x7 = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
  auto x8 = CD3DX12_RESOURCE_DESC::Buffer(byteSize);
  // Create the actual default buffer resource.
  ThrowIfFailed(device->CreateCommittedResource(
      &x7, D3D12_HEAP_FLAG_NONE, &x8, D3D12_RESOURCE_STATE_COMMON, nullptr,
      IID_PPV_ARGS(defaultBuffer.GetAddressOf())));

  // In order to copy CPU memory data into our default buffer, we need to create
  // an intermediate upload heap.
  auto x9 = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
  auto x10 = CD3DX12_RESOURCE_DESC::Buffer(byteSize);
  ThrowIfFailed(device->CreateCommittedResource(
      &x9, D3D12_HEAP_FLAG_NONE, &x10, D3D12_RESOURCE_STATE_GENERIC_READ,
  ...
  auto x2 = CD3DX12_RESOURCE_BARRIER::Transition(
      defaultBuffer.Get(), D3D12_RESOURCE_STATE_COMMON,
      D3D12_RESOURCE_STATE_COPY_DEST);
  cmdList->ResourceBarrier(1, &x2);
