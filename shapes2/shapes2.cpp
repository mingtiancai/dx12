...
void ShapesApp::Update(const GameTimer &gt) {
  ...
  if (mCurrFrameResource->Fence != 0 &&
    ...
    HANDLE eventHandle =
        CreateEventEx(nullptr, LPCWSTR(false), false, EVENT_ALL_ACCESS);
    ThrowIfFailed(
...
void ShapesApp::Draw(const GameTimer &gt) {
  ...
  auto x3 = CD3DX12_RESOURCE_BARRIER::Transition(
      CurrentBackBuffer(), D3D12_RESOURCE_STATE_PRESENT,
      D3D12_RESOURCE_STATE_RENDER_TARGET);
  mCommandList->ResourceBarrier(1, &x3);
  ...
  auto x1 = CurrentBackBufferView();
  auto x2 = DepthStencilView();
  mCommandList->OMSetRenderTargets(1, &x1, true, &x2);
  ...
  mCommandList->ResourceBarrier(1, &x4);
...
void ShapesApp::UpdateMainPassCB(const GameTimer &gt) {
  ...
  auto x5 = XMMatrixDeterminant(view);
  XMMATRIX invView = XMMatrixInverse(&x5, view);
  auto x6 = XMMatrixDeterminant(proj);
  XMMATRIX invProj = XMMatrixInverse(&x6, proj);
  auto x7 = XMMatrixDeterminant(viewProj);
  XMMATRIX invViewProj = XMMatrixInverse(&x7, viewProj);
...
void ShapesApp::BuildShadersAndInputLayout() {
  mShaders["standardVS"] =
      d3dUtil::CompileShader(L"Shaders\\color.hlsl", nullptr, "VS", "vs_5_1");
  mShaders["opaquePS"] =
      d3dUtil::CompileShader(L"Shaders\\color.hlsl", nullptr, "PS", "ps_5_1");
...
void ShapesApp::DrawRenderItems(ID3D12GraphicsCommandList *cmdList,
  ...
  for (size_t i = 0; i < ritems.size(); ++i) {
    ...
    auto x9 = ri->Geo->VertexBufferView();
    auto x10 = ri->Geo->IndexBufferView();
    cmdList->IASetVertexBuffers(0, 1, &x9);
    cmdList->IASetIndexBuffer(&x10);
    cmdList->IASetPrimitiveTopology(ri->PrimitiveType);

    // Offset to the CBV in the descriptor heap for this object and for this
    // frame resource.
    UINT cbvIndex =
        mCurrFrameResourceIndex * (UINT)mOpaqueRitems.size() + ri->ObjCBIndex;
