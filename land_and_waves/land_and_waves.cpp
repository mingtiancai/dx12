...
void LandAndWavesApp::UpdateMainPassCB(const GameTimer &gt) {
  ...
  auto x8 = XMMatrixDeterminant(viewProj);
  XMMATRIX invViewProj = XMMatrixInverse(&x8, viewProj);
...
void LandAndWavesApp::BuildWavesGeometryBuffers() {
  std::vector<std::uint16_t> indices(
      3 * mWaves->TriangleCount()); // 3 indices per face
...
void LandAndWavesApp::BuildRenderItems() {
  ...
  wavesRitem->StartIndexLocation =
      wavesRitem->Geo->DrawArgs["grid"].StartIndexLocation;
  wavesRitem->BaseVertexLocation =
      wavesRitem->Geo->DrawArgs["grid"].BaseVertexLocation;
...
void LandAndWavesApp::DrawRenderItems(ID3D12GraphicsCommandList *cmdList,
  ...
  for (size_t i = 0; i < ritems.size(); ++i) {
    ...
    auto x11 = ri->Geo->VertexBufferView();
    auto x12 = ri->Geo->IndexBufferView();
    cmdList->IASetVertexBuffers(0, 1, &x11);
    cmdList->IASetIndexBuffer(&x12);
